...
| `secureId` | Required if `external_id` is not supplied | WestraPay transaction `secureId` returned during initiation. You can also use `secure_id`. |
| `external_id` | Required if `secureId` is not supplied | Your unique transaction reference sent during initiation. You can also use `externalId`. |
...
```bash
curl --location 'https://connect.westrapay.com/api/v1/transaction?merchant=your_merchant_id&external_id=merchant-rw-payin-10001' \
  --header 'Authorization: Bearer YOUR_TOKEN_HERE'
```
...
- Query by `secureId` when available because it is generated by WestraPay and uniquely identifies the transaction.
- You can also query by your own `external_id` as long as you include your merchant ID.
- Continue to store both `secureId` and your own `external_id` for matching the transaction to your order.
- The `direction` field is `payin` for collections and `payout` for withdrawals.
